 ...
 *
 * Spawns a separate `pi` process for each subagent invocation, giving it its
 * own context window. The parent only receives the child's compressed summary,
 * so raw file reads and search output never enter the parent's context.
 *
 * Modes:
 *   - single:   { agent, task }
 ...
 *
 * Agent definitions live in ~/.pi/agent/agents/*.md (user scope, default) and
 * .pi/agents/*.md (project scope, opt-in). Bundled agents (scout, reviewer)
 * declare read-only tool allowlists that exclude `subagent`, so a child cannot
 * re-delegate — the recursion guard is the agent definitions themselves.
 *
 * Ported from the official pi example at
...
// --- Agent discovery (inlined; mirrors the reference's agents.ts) -----------
...
// --- Child spawn + JSON event parsing ---------------------------------------
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (params.chain && params.chain.length > 0) {
        ...
        for (let i = 0; i < params.chain.length; i++) {
          ...
          // Update callback that combines completed steps with the streaming one.
