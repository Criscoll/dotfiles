...
export default async function (pi: ExtensionAPI) {
  ...
  pi.on("tool_call", async (event, ctx) => {
    try {
      ...
      // rg -l / --files-with-matches: rtk grep -l delegates to system grep -l which fails
      // on directories (no -r). Let real rg handle it.
      if (/\brg\b.*(?:-[a-zA-Z]*l\b|--files-with-matches)/.test(cmd)) return

      // rg -r / --replace: if rewritten to `rtk grep -r`, the inefficient-commands hook
      // then blocks it as "grep -r is slow". Let inefficient-commands catch it instead.
      if (/\brg\b.*(?:-[a-zA-Z]*r\b|--replace)/.test(cmd)) return

      // rg with \| alternation: rg uses | not \|. Let inefficient-commands explain this.
