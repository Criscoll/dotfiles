...
Both backends below run as Docker containers on the VPS (`134.199.169.64`, hostname `SploomVPS`). The SSH tunnel is only needed when connecting *from a different machine*. When Claude Code is running directly on the VPS itself, both services are already reachable on `localhost` at the ports below and no tunnel is required.

**Current user-scoped MCPs:**

| Name | Transport | URL / Command | Prerequisites |
|---|---|---|---|
| `c4ai-sse` | SSE | `http://localhost:11235/mcp/sse` | On the VPS: none, already local. Elsewhere: SSH tunnel `ssh -L 11235:localhost:11235 -N cristian@134.199.169.64` |
| `playwright` | SSE | `http://localhost:8931/sse` | On the VPS: none, already local. Elsewhere: SSH tunnel `ssh -L 8931:localhost:3001 -N cristian@134.199.169.64` |
