...
description: >-
  ...
  hands-off: dispatching sub-agents in tier-and-surface groups at the effort tier the plan
  names, deciding and logging forks, committing per item, and — when an item can't be completed — gathering
...
- **The orchestrator delegates every step; it never implements inline.** `EXECUTE.md` dispatches `PLAN.md` steps to sub-agents in **groups** — consecutive steps sharing a tier and a code surface, briefed together and reviewed as one diff — never inline by the orchestrator. A contract-producing step is dispatched alone. The orchestrator's own job is dispatching, reviewing diffs against the plan, deciding forks, gating, committing, and logging. Only trivial review fixups (about 5 lines or fewer) are applied inline by the orchestrator itself.
